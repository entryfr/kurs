...
В интерфейсе intentionally оставлены только поля «картинка + запрос».
Проверка LLM выполняется через API `POST /api/llm/check`.
...
Файл `.env.local` уже добавлен в `.gitignore`, в репозиторий он не попадёт.
